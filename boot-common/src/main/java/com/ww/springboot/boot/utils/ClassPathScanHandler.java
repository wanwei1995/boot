...
public class ClassPathScanHandler {  
     ...
     * @return Set 
     */  
    public Set<Class<?>> getPackageAllClassesByInterface(String basePackage,  
            boolean recursive, Class<?> interfaceClazz) { 
    ...
    private void doScanPackageClassesByJar(String basePackage, URL url,  
            final boolean recursive, Set<Class<?>> classes, Class<?> interfaceClazz) {  
        String packageName = basePackage;  
        String package2Path = packageName.replace('.', '/');  
        JarFile jar;  
        try {  
            ...
            while (entries.hasMoreElements()) {  
                ...
                if (this.filterClassName(classSimpleName)) {  
                    ...
                    try {  
                        doAddClass(classes, className, interfaceClazz);
    ...
    private void doScanPackageClassesByFile(Set<Class<?>> classes,  
            String packageName, String packagePath, boolean recursive, Class<?> interfaceClazz) {  
        ...
        for (File file : dirfiles) {  
            if (file.isDirectory()) {  
                doScanPackageClassesByFile(classes, packageName + "."  
                        + file.getName(), file.getAbsolutePath(), recursive, interfaceClazz);  
            } else {  
                String className = file.getName().substring(0,  
                        file.getName().length() - 6);  
                try {  
                	doAddClass(classes, packageName + '.' + className, interfaceClazz);
    ...
    private static void doAddClass(Set<Class<?>> classSet, String className, Class<?> interfaceClazz) throws ClassNotFoundException {
    	Class<?> cls = Thread.currentThread().getContextClassLoader().loadClass(className);
		if (interfaceClazz.isAssignableFrom(cls)) {
			if (!interfaceClazz.equals(cls)) {
				// 自身并不加进去
				classSet.add(cls);
			}
		}
