...
public class ExcelUtil {
     ...
     * @return 返回-1 表示所在行为空
     */
    public static int getColumnCount(int sheetIx, int rowIndex,Workbook workbook) {
        Sheet sheet = workbook.getSheetAt(sheetIx);
        Row row = sheet.getRow(rowIndex);
    ...
    public static String convertCellStr(Cell cell) {
		...
		if (cell == null) {
			...
			switch (cell.getCellType()) {
				case 2 :
					if (HSSFDateUtil.isCellDateFormatted(cell)) {
						cellStr = DateUtil.dateToString(cell.getDateCellValue());
					} else {
						DecimalFormat evaluator1 = new DecimalFormat("0");
						cellStr = String.valueOf(evaluator1.format(cell.getNumericCellValue()));
					}
					break;
				case 3 :
				...
				case 5 :
				case 0 :if (HSSFDateUtil.isCellDateFormatted(cell)) {
	                Date date = cell.getDateCellValue();
	                if (pattern != null) {
	                    SimpleDateFormat sdf = new SimpleDateFormat(pattern);
	                    cellStr = sdf.format(date);
	                } else {
	                	cellStr = date.toString();
	                }
	                break;
	            }
	            // 不是日期格式，则防止当数字过长时以科学计数法显示
	            ...
	            break;
				case 1 : cellStr = cell.getStringCellValue();
				break;
				default :
					break;
				case 6 :
			...
			return cellStr == null ? null : cellStr.trim();
		}
	}
